...
for lat in range(lenlat):
    for lon in range(lenlon):
        ...
        LE=np.array(mat2[var_EF][summer_mask,lat,lon])
        ...
        try:
            p , e = optimize.curve_fit(piecewise_linear,SM,LE, p0=[(np.max(SM)+np.min(SM))/2, np.median(LE), 0, 1], bounds=([np.min(SM), 0,   -0.001,  0.001], [np.max(SM), 1,    0.001,   10]))     
            # p , e= optimize.curve_fit(piecewise_linear, SM, LE,p0=[(np.max(SM)+np.min(SM))/2,np.median(LE),0,50], bounds=([np.min(SM),0,-0.001,0.001], [np.max(SM),np.max(LE),0.001,50])) # previous bounds
        ...
        try:        
            p , e = optimize.curve_fit(piecewise_linear, SM, LE,p0=[(np.max(SM)+np.min(SM))/2, np.median(LE), 1, 0], bounds=([np.min(SM), 0,    0.001, -0.001], [np.max(SM), 1,    10,      0.001]))
            # p , e= optimize.curve_fit(piecewise_linear, SM, LE,p0=[(np.max(SM)+np.min(SM))/2,np.median(LE),1,0], bounds=([np.min(SM),0,0.001,-0.001], [np.max(SM),1,50,0.001])) # previous bounds
        ...
        try:        
            p , e= optimize.curve_fit(piecewise3sg_linear, np.array(SM), np.array(LE),p0=[np.percentile(np.array(SM),25),np.percentile(np.array(SM),75),np.percentile(np.array(LE),25),np.percentile(np.array(LE),75),0,1,0], bounds=([np.min(SM), np.min(SM),  0,   0,   -0.001,  0.001, -0.001], [np.max(SM), np.max(SM),  1,   1,    0.001,  10,      0.001]))
            # p , e= optimize.curve_fit(piecewise3sg_linear, np.array(SM), np.array(LE),p0=[np.percentile(np.array(SM),25),np.percentile(np.array(SM),75),np.percentile(np.array(LE),25),np.percentile(np.array(LE),75),0,1,0], bounds=([np.min(np.array(SM)),np.min(np.array(SM)),np.min(np.array(LE)),np.min(np.array(LE)),-0.001,0.001,-0.001], [np.max(np.array(SM)),np.max(np.array(SM)),np.max(np.array(LE)),np.max(np.array(LE)),0.001,50,0.001])) # previous bounds
...
ncfilenm=path_outputs + '/BP_SMsxEF_' + name_land + '_JJA_optimized_new_bounds.nc'
