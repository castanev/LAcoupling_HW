...
def event_severity_cluster(anom, track, grid_cell_area_km2, dates_d, path_figure, lats_arr, lons_arr, half_box_deg=1.0):
    """Severity = AUC of daily area-weighted mean positive SAT anomaly in a COM-centered box.

    Each event day:
        daily_mean = ∑ (max(T', 0) · A_cell) / ∑ A_cell
                     over a (2*half_box_deg)° × (2*half_box_deg)° box
                     centered on that day's tracked COM.
    ...
    lats_arr = np.asarray(lats_arr, dtype=float)
    lons_arr = np.asarray(lons_arr, dtype=float)

    lags = []
    dates_event = []
    daily_means = []
    onset_day = track['obs'][0]['day']
    for obs, _mask in zip(track['obs'], track['cluster_masks']):
        day = obs['day']
        lat_c = float(obs['lat'])
        lon_c = float(obs['lon'])
        lat_mask = np.where(
        ...
        if lat_mask.size == 0:
            lat_mask = np.array([int(np.argmin(np.abs(lats_arr - lat_c)))])
        if lon_mask.size == 0:
            lon_mask = np.array([int(np.argmin(np.abs(lons_arr - lon_c)))])

        box_tprime = np.asarray(anom[day], dtype=float)[np.ix_(lat_mask, lon_mask)]
        box_area = area[np.ix_(lat_mask, lon_mask)]
        valid = np.isfinite(box_tprime) & np.isfinite(box_area)
        if not np.any(valid):
            continue

        pos = np.maximum(box_tprime, 0.0)
        area_sum = float(np.nansum(box_area[valid]))
        if area_sum <= 0:
            continue
        daily_mean = float(np.nansum(pos[valid] * box_area[valid]) / area_sum)
...
def main():
    ...
    for event in events:
        ...
        out_name = f"severity_cluster_dailyCOM2degreesbox_{event['date'].strftime('%Y%m%d')}_idx{onset}.png"
        out_path = os.path.join(path_figures, out_name)
        event_severity = event_severity_cluster(
            anom_us, track, grid_cell_area, dates_d, out_path, lats_us, lons_us,
        )
