...
#   T'_HW(x,y) = α(x,y) + β(x,y) SM'(x,y, t0−L) + ε
# where SM' is the pre-heatwave soil-moisture anomaly (lead window before onset)
...
parser.add_argument(
    '--lead_start', type=int, default=5,
    help='Earliest lead day before onset (positive; default 5 → day t0−5)',
)
parser.add_argument(
    '--lead_end', type=int, default=1,
    help='Latest lead day before onset (positive; default 1 → day t0−1)',
)
parser.add_argument(
...
lead_start = int(args.lead_start)
lead_end = int(args.lead_end)
min_n = int(args.min_n)
if lead_start < lead_end or lead_start < 1 or lead_end < 1:
    raise ValueError(
        f'Need lead_start >= lead_end >= 1 (got start={lead_start}, end={lead_end})'
    )
...
for _, row in df_hw_events.iterrows():
    ...
    # Pre-HW lead window [t0−lead_start, t0−lead_end]
    ism_lead = []
    for k in range(lead_end, lead_start + 1):
        d = onset - pd.Timedelta(days=k)
        if d in date_to_ism:
            ism_lead.append(date_to_ism[d])
    if not ism_lead:
        n_skip_no_sm += 1
        continue
    ...
    sm_lead = sm_vals[np.asarray(ism_lead)]
    with np.errstate(invalid='ignore'):
        sm_pre = np.nanmean(sm_lead, axis=0)
...
print(
    ...
    f'skip no SM lead (e.g. pre-1980)={n_skip_no_sm}, '
...
print(
    ...
    f'lead=−{lead_start}…−{lead_end} d, min_n={min_n}, '
...
lead_tag = f'lead{lead_start}to{lead_end}'
...
plot_clim_map(
    beta,
    f'{path_figures}HW_SMtoT_beta_{lead_tag}_{variable_sm}_{name_land}.png',
    title=rf'HW-specific SM→T sensitivity $\beta$ (SM lead −{lead_start}…−{lead_end} d)',
...
plot_clim_map(
    r,
    f'{path_figures}HW_SMtoT_r_{lead_tag}_{variable_sm}_{name_land}.png',
    title=rf'HW SM→T correlation $r$ (lead −{lead_start}…−{lead_end} d)',
    cbar_label=r'$r$',
    vmax=vmax_r,
    cmap='RdBu_r',
...
plot_clim_map(
    n_pix,
    f'{path_figures}HW_SMtoT_n_{lead_tag}_{variable_sm}_{name_land}.png',
...
out_nc = (
    f'{path_outputs}{case}/HW_SMtoT_sensitivity_{lead_tag}_{variable_sm}_{name_land}.nc'
)
xr.Dataset(
    {
    ...
    attrs={
        'description': 'Heatwave-specific SM→T sensitivity: T_HW = alpha + beta*SM_pre',
        'SM_lead_days': f'-{lead_start} to -{lead_end}',
        'min_n': min_n,
