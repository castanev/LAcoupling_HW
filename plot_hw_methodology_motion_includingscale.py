...
def parse_events_csv(path_csv, years=None):
    df = pd.read_csv(path_csv, index_col=0)
    years_set = None if years is None else set(years)
    events = []
    for onset_idx, row in df.iterrows():
        date = pd.to_datetime(row['Date 0'])
        if years_set is not None and date.year not in years_set:
            continue
        events.append({
...
def event_severity_cluster_including_scale(anom, track, grid_cell_area_km2, dates_d, path_figure, area_ref):
    """Severity including spatial scale, then normalized by a climatological HW area.

    Each event day:
        daily = ∑ (max(T', 0) · A_cell) / area_ref
              = (A_HW(t) / area_ref) · area-weighted mean of max(T', 0)

    Then sum over event days (Δt = 1 day). Units: °C·day.
    Larger-than-typical footprints raise the daily contribution.
    """
    if track is None or not np.isfinite(area_ref) or area_ref <= 0:
        return np.nan
    ...
    for obs, mask in zip(track['obs'], track['cluster_masks']):
        ...
        if area_sum <= 0:
            continue
        daily_mean = float(np.nansum(pos[m] * area[m]) / area_ref)
...
def plot_event_severity_cluster(lags, dates_event, daily_means, event_severity, path_figure):
    ...
    ax.set_ylabel(r"$(A_{\mathrm{HW}}/A_{\mathrm{ref}})\,\overline{T'}$ [°C]", fontsize=13)
    ax.set_title('Daily cluster anomaly × relative footprint (severity = AUC)', fontsize=12)
...
def calculate_area_ref(tracks, events, grid_cell_area):
    """Mean tracked footprint over every catalog HW day (all events).

    A_HW(t) = ∑ A_cell over that day's cluster mask.
    area_ref = mean of those daily areas. Longer events contribute more days.
    """
    area = np.asarray(grid_cell_area, dtype=float)
    daily_areas = []
    n_matched = 0
    for event in events:
        track = find_track_for_onset(tracks, event['onset_idx'])
        if track is None:
            continue
        n_matched += 1
        for mask in track['cluster_masks']:
            m = np.squeeze(mask).astype(bool)
            if not np.any(m):
                continue
            a = float(np.nansum(area[m]))
            if a > 0:
                daily_areas.append(a)
    if not daily_areas:
        raise ValueError('No valid daily cluster areas to compute area_ref.')
    area_ref = float(np.mean(daily_areas))
    print(
        f'area_ref: mean daily footprint = {area_ref:.0f} km² '
        f'(n_days={len(daily_areas)}, n_events={n_matched}, '
        f'median={np.median(daily_areas):.0f} km²)'
    )
    return area_ref
...
def main():
    ...
    events_all = parse_events_csv(events_csv, years=None)
    area_ref = calculate_area_ref(tracks, events_all, grid_cell_area)
    ...
    for event in events:
        ...
        out_name = f"severity_scale_{event['date'].strftime('%Y%m%d')}_idx{onset}.png"
        out_path = os.path.join(path_figures, out_name)
        event_severity = event_severity_cluster_including_scale(
            anom_us, track, grid_cell_area, dates_d, out_path, area_ref,
        )
