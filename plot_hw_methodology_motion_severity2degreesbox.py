...
def plot_event_figure(
    ...
    day_offsets = [-1, 0, 1, 2]
    day_labels = ['Day -1', 'Onset (day 0)', 'Day +1', 'Day +2']
...
def plot_event_severity_cluster(lags, dates_event, daily_means, event_severity, path_figure):
    ...
    ax.axvline(0.0, color='0.25', ls='--', lw=1.0, label='Onset')
    ax.set_xlabel('Days since onset', fontsize=12)
    ax.set_ylabel("Area-weighted mean max(T', 0) [°C]", fontsize=12)
    ...
    ax.text(
        ...
        f"{dates_event[0].strftime('%Y-%m-%d')} → {dates_event[-1].strftime('%Y-%m-%d')}",
...
def main():
    ...
    for event in events:
        onset = event['onset_idx']
        if onset - 1 < 0 or onset + 2 >= t_us.shape[0]:
