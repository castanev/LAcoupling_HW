...
For each event in selected calendar years, produces:
  - a 2x4 panel figure (tracked cluster + temperature anomalies)
  - a severity time series based on daily cluster-mean temperature exceedance
    above the saved US threshold (severity = area under the curve)
...
def parse_args():
    ...
    return argparse.Namespace(
        ...
        path_outputs_case=cfg.get('path_outputs_case', cfg['path_case_land']),
...
def load_us_threshold(path_threshold):
    """Load summer day-of-year threshold field saved by the detection script."""
    with Dataset(path_threshold) as ncfile:
        lats_th = np.array(ncfile['lat'][:])
        lons_th = np.array(ncfile['lon'][:])
        threshold = np.array(ncfile['threshold'][:], dtype=float)
        time_str = np.array([str(t) for t in ncfile['time'][:]])

    days_summer = np.array([pd.to_datetime(t, format='%Y%m%d') for t in time_str])
    if threshold.ndim == 2:
        threshold = threshold[np.newaxis, :, :]
    df_threshold = pd.DataFrame(
        index=days_summer,
        data=np.reshape(threshold, (len(days_summer), threshold.shape[1] * threshold.shape[2])),
    )
    return threshold, df_threshold, lats_th, lons_th
...
def event_severity_exceedance(
    t_us, track, grid_cell_area_km2, dates_d, df_threshold, threshold, path_figure,
):
    """Severity = AUC of daily cluster-mean temperature exceedance above threshold.

    For each event day:
        1. Use only pixels in that day's tracked cluster mask.
        2. Exceedance = max(T - threshold, 0) at those pixels.
        3. Daily value = area-weighted mean exceedance over the cluster.
    Severity = sum of daily values (Δt = 1 day).
    ...
    if t_us.shape[1:] != area.shape:
        raise ValueError(
            f'Temperature grid {t_us.shape[1:]} does not match area grid {area.shape}.'
        )
    ...
    for obs, mask in zip(track['obs'], track['cluster_masks']):
        ...
        if not np.any(m):
            continue
        thr = threshold_for_day(day, dates_d, df_threshold, threshold)
        t_day = np.asarray(t_us[day], dtype=float)
        exceed = np.maximum(t_day - thr, 0.0)
        area_sum = float(np.nansum(area[m]))
        if area_sum <= 0:
            continue
        daily_mean = float(np.nansum(exceed[m] * area[m]) / area_sum)
    ...
    plot_event_severity_exceedance(lags, dates_event, daily_means, total, path_figure)
    return total


def plot_event_severity_exceedance(lags, dates_event, daily_means, event_severity, path_figure):
    """Time series of footprint-mean temperature exceedance; AUC is severity."""
    ...
    ax.set_ylabel('Mean temperature exceedance [°C]', fontsize=13)
    ax.set_title(
        'Daily cluster-mean exceedance above threshold (severity = area under curve)',
        fontsize=12,
    )
    ax.set_xlim(lags.min() - 0.3, lags.max() + 0.3)
    ymax = float(np.nanmax(daily_means)) if daily_means.size else 2.0
    ylim_top = max(1.0, np.ceil((ymax + 0.8) / 1.0) * 1.0)
...
def main():
    ...
    path_outputs_case = args.path_outputs_case or args.path_case
    path_threshold = os.path.join(path_outputs_case, 'US_threshold.nc')
    if os.path.exists(path_threshold):
        threshold, df_threshold, lats_th, lons_th = load_us_threshold(path_threshold)
        if (
            not np.allclose(lats_th, lats_us)
            or not np.allclose(lons_th, lons_us)
        ):
            raise ValueError(
                f'Grid mismatch between {path_threshold} and temperature field.'
            )
        print(f'Loaded threshold from {path_threshold}')
    else:
        print(f'{path_threshold} not found; computing threshold on the fly')
    ...
    for event in events:
        ...
        out_name = f"severity_exceedance_{event['date'].strftime('%Y%m%d')}_idx{onset}.png"
        out_path = os.path.join(path_figures, out_name)
        event_severity = event_severity_exceedance(
            t_us, track, grid_cell_area, dates_d, df_threshold, threshold, out_path,
        )
