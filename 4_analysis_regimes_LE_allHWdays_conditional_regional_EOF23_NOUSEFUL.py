...
# =======================================================
# EOF2 or EOF3 negative-phase years (late April)
# Union of years with PC2* < -thresh and/or PC3* < -thresh.
# =======================================================
...
year_neg_by_period = {}
year_neg_by_eof = {}  # eof_index (1-based) -> years for lateApr
for eof_period_label, eof_sel_month, eof_day_lo, eof_day_hi in eof_periods:
    ...
    for EOF_i in EOF_indices_neg:
        ...
        years_union = (
            year_neg if years_union is None
            else np.union1d(years_union, year_neg)
        )
    year_neg_by_period[eof_period_label] = np.asarray(years_union, dtype=int)
    print(
        f'EOF2∪EOF3 {eof_period_label} negative phase: '
        f'n={len(years_union)} years → {years_union.tolist()}'
    )
ds_eof.close()

# Tag each HW event for late-April EOF2-neg, EOF3-neg, and their union
hw_csv_path = f'{path_case}{name_file_posHW}'
year_neg_eof2 = set(int(y) for y in year_neg_by_eof[2])
year_neg_eof3 = set(int(y) for y in year_neg_by_eof[3])
year_neg_union = set(int(y) for y in year_neg_by_period['lateApr'])
onset_idx = df_heatwaves.index.values.astype(int)
lateApr_eof2_neg = np.zeros(len(onset_idx), dtype=bool)
lateApr_eof3_neg = np.zeros(len(onset_idx), dtype=bool)
lateApr_eof2or3_neg = np.zeros(len(onset_idx), dtype=bool)
valid_onset = onset_idx < len(dates_d)
for j, day0 in enumerate(onset_idx):
    if not valid_onset[j]:
        continue
    y = int(dates_d[int(day0)].year)
    lateApr_eof2_neg[j] = y in year_neg_eof2
    lateApr_eof3_neg[j] = y in year_neg_eof3
    lateApr_eof2or3_neg[j] = y in year_neg_union
df_heatwaves['LateApri_EOF2'] = lateApr_eof2_neg
df_heatwaves['LateApri_EOF3'] = lateApr_eof3_neg
df_heatwaves['LateApri_EOF2or3'] = lateApr_eof2or3_neg
df_heatwaves.to_csv(hw_csv_path)
print(
    f"Updated {hw_csv_path} with LateApri_EOF2 / LateApri_EOF3 / LateApri_EOF2or3 "
    f"(union True: {int(lateApr_eof2or3_neg.sum())}/{len(lateApr_eof2or3_neg)}; "
    f"years={sorted(year_neg_union)})"
)
...
# Run density for late-April EOF2∪EOF3-neg years, restricted to south-central US
...
for eof_period_label, year_neg_union in year_neg_by_period.items():
    year_neg_set = set(int(y) for y in np.asarray(year_neg_union))
    ...
    cond_tag = (
        f'EOF2or3neg_{eof_period_label}_thresh{pc_thresh:g}_{region_fig}'
    )
    print(
        f'\n=== Regime density | {cond_tag}: '
        f'{event_mask.sum()} / {event_mask.size} HW events '
        f'(EOF2∪EOF3 filter={year_mask.sum()}, region filter={region_mask.sum()}) ==='
    )
    ...
    for lag_days in lags_days:
        ...
        if os.path.exists(density_cache_path):
            ...
            if (
                ...
                print(
                    f'Cache missing regime-3/4 or all-regime fields; recomputing '
