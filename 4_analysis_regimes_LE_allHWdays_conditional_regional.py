...
if region == 'US':
    ...
    lat_minHW = 25; lat_maxHW = 40; lon_minHW = 252; lon_maxHW = 270; midlat=45 #-125 to -100   SOUTH-CENTRAL
...
# =======================================================
# EOF2 / EOF3 negative-phase years (late April)
# Same recipe as 9_analysis_EOF_AprMay.py; run each EOF separately.
# =======================================================
...
# eof_num (1-based) -> negative-phase years for lateApr
year_neg_by_eof = {}
...
# Tag each HW event for late-April EOF2-neg and EOF3-neg
hw_csv_path = f'{path_case}{name_file_posHW}'
onset_idx = df_heatwaves.index.values.astype(int)
valid_onset = onset_idx < len(dates_d)
onset_years = np.full(len(onset_idx), -1, dtype=int)
onset_years[valid_onset] = np.array(
    [int(dates_d[int(day0)].year) for day0 in onset_idx[valid_onset]]
)
for eof_num, year_neg in year_neg_by_eof.items():
    year_set = set(int(y) for y in year_neg)
    flag = np.array([y in year_set for y in onset_years], dtype=bool)
    col = f'LateApri_EOF{eof_num}'
    df_heatwaves[col] = flag
    print(
        f"Updated {hw_csv_path} with {col} "
        f"({int(flag.sum())}/{len(flag)} events True; "
        f"lateApr EOF{eof_num}-neg years={sorted(year_set)})"
    )
...
def _plot_density_panel(
    ...
    spearman_corr=False,
    blue_cbar_label=None,
    red_cbar_label=None,
):
    if blue_cbar_label is None:
        blue_cbar_label = f"Summer density (0–{density_vmax:.2f})"
    if red_cbar_label is None:
        red_cbar_label = "HW-day density"
    ...
    if has_summer and sm_summer_valid.size > 5:
        ...
        cbar_s.set_label(blue_cbar_label, fontsize=10)
    ...
    if show_hw_contours and has_hw and sm_hw_valid.size > 5:
        ...
        cbar_hw.set_label(red_cbar_label, fontsize=10)
...
# Run density for late-April EOF2-neg and EOF3-neg years, south-central US
# Analysis box (independent of --region for event filtering / figure tags)
lat_min_box, lat_max_box = 25.0, 40.0
lon_min_box, lon_max_box = 252.0, 270.0
region_fig = 'southcentralUS'
...
for eof_num, year_neg in year_neg_by_eof.items():
    ...
    region_mask = np.array([
        (lat_min_box <= float(lat) <= lat_max_box)
        and (lon_min_box <= _lon360(lon) <= lon_max_box)
        for lat, lon in zip(lats_events_all, lons_events_all)
    ], dtype=bool)
    event_mask = year_mask & region_mask
    lats_events = np.asarray(lats_events_all)[event_mask]
    lons_events = np.asarray(lons_events_all)[event_mask]
    dates_d_HW_day0 = dates_onset_all[event_mask]
    duration_events = duration_events_all[event_mask]
    cond_tag = (
        f'EOF{eof_num}neg_{eof_period_label}_thresh{pc_thresh:g}_{region_fig}'
    )
    print(
        f'\n=== Regime density | {cond_tag}: '
        f'{event_mask.sum()} / {event_mask.size} HW events '
        f'(EOF{eof_num} filter={year_mask.sum()}, region filter={region_mask.sum()}) ==='
    )

    density_cache_path = (
        f'{path_outputs_case}density_data_{variable_LE}_{variable_sm}_'
        f'summer_HWdays_{cond_tag}_05deg_{name_land}.npz'
    )
    ...
    for lag_days in lags_days:
        ...
        if os.path.exists(density_cache_path):
            ...
            if (
                'r3_sm_summer' in cache.files
                and 'r4_sm_summer' in cache.files
                and 'all_sm_summer' in cache.files
                and 'all_sm_jja_cond' in cache.files
                and 'all_sm_jja_rest' in cache.files
            ):
                load_cache = True
            else:
                print(
                    f'Cache missing regime/all/JJA-year fields; recomputing '
        ...
        if load_cache:
            ...
            all_sm_summer = cache['all_sm_summer']
            all_LE_summer = cache['all_LE_summer']
            all_sm_hw = cache['all_sm_hw']
            all_LE_hw = cache['all_LE_hw']
            n_hw_all = int(cache['n_hw_all'])
            all_sm_jja_cond = cache['all_sm_jja_cond']
            all_LE_jja_cond = cache['all_LE_jja_cond']
            all_sm_jja_rest = cache['all_sm_jja_rest']
            all_LE_jja_rest = cache['all_LE_jja_rest']
            n_evt_jja_years = int(cache['n_evt_jja_years'])
        else:
            ...
            # All events, absolute SM (no breakpoint / regime split)
            all_sm_summer, all_LE_summer = [], []
            all_sm_hw, all_LE_hw = [], []
            # JJA by year type at the same event locations (abs SM)
            all_sm_jja_cond, all_LE_jja_cond = [], []
            all_sm_jja_rest, all_LE_jja_rest = [], []
            n_evt_jja_years = 0
            ...
            for lat_center_hw, lon_center_hw, onset_day, duration_event in zip(
                ...
                sm_abs_aligned = sm_abs[np.isin(dates_sm, common_dates)]
                ...
                sm_summer_abs = sm_abs_aligned[summer_mask]
                valid_all = np.isfinite(LE_summer) & np.isfinite(sm_summer_abs)
                if np.any(valid_all):
                    all_LE_summer.append(LE_summer[valid_all])
                    all_sm_summer.append(sm_summer_abs[valid_all])
                if len(heatwave_idx) > 0:
                    LE_hw = LE_aligned[heatwave_idx]
                    sm_hw_abs = sm_abs_aligned[heatwave_idx]
                    valid_hw_all = np.isfinite(LE_hw) & np.isfinite(sm_hw_abs)
                    if np.any(valid_hw_all):
                        all_LE_hw.append(LE_hw[valid_hw_all])
                        all_sm_hw.append(sm_hw_abs[valid_hw_all])

                # JJA conditioned years (red) vs rest of years (blue)
                jja_mask = np.array([d.month in (6, 7, 8) for d in common_dates])
                in_cond_year = np.array(
                    [int(d.year) in year_neg_set for d in common_dates], dtype=bool
                )
                cond_jja = jja_mask & in_cond_year
                rest_jja = jja_mask & (~in_cond_year)
                LE_cond = LE_aligned[cond_jja]
                sm_cond = sm_abs_aligned[cond_jja]
                valid_cond = np.isfinite(LE_cond) & np.isfinite(sm_cond)
                LE_rest = LE_aligned[rest_jja]
                sm_rest = sm_abs_aligned[rest_jja]
                valid_rest = np.isfinite(LE_rest) & np.isfinite(sm_rest)
                if np.any(valid_cond) or np.any(valid_rest):
                    n_evt_jja_years += 1
                if np.any(valid_cond):
                    all_LE_jja_cond.append(LE_cond[valid_cond])
                    all_sm_jja_cond.append(sm_cond[valid_cond])
                if np.any(valid_rest):
                    all_LE_jja_rest.append(LE_rest[valid_rest])
                    all_sm_jja_rest.append(sm_rest[valid_rest])

                # Regime-separated samples (breakpoint-relative SM for regimes 3/4)
            ...
            all_sm_hw = _as_1d_points(all_sm_hw)
            all_LE_hw = _as_1d_points(all_LE_hw)
            all_sm_jja_cond = _as_1d_points(all_sm_jja_cond)
            all_LE_jja_cond = _as_1d_points(all_LE_jja_cond)
            all_sm_jja_rest = _as_1d_points(all_sm_jja_rest)
            all_LE_jja_rest = _as_1d_points(all_LE_jja_rest)
            ...
            np.savez_compressed(
                ...
                r4_LE_hw=r4_LE_hw,
                n_hw_r4=n_hw_r4,
                all_sm_summer=all_sm_summer,
                all_LE_summer=all_LE_summer,
                all_sm_hw=all_sm_hw,
                all_LE_hw=all_LE_hw,
                n_hw_all=n_hw_all,
                all_sm_jja_cond=all_sm_jja_cond,
                all_LE_jja_cond=all_LE_jja_cond,
                all_sm_jja_rest=all_sm_jja_rest,
                all_LE_jja_rest=all_LE_jja_rest,
                n_evt_jja_years=n_evt_jja_years,
            )
        ...
        _plot_density_panel(
            ...
            outfile=(
                ...
                f"05deg_trans_{cond_tag}_{name_land}.png"
            ),
            show_critical_vline=False,
            density_vmax=0.6,
        ...
        # All events pooled: absolute SM, no regime / breakpoint split
        _plot_density_panel(
            all_sm_summer,
            all_LE_summer,
            all_sm_hw,
            ...
            xlabel=r"SMs (regional mean, ±0.5°)",
            title=(
                f"SM vs LE — all regimes pooled (absolute SM, no breakpoint)\n"
                f"south-central US | HW events in {cond_tag}"
            ),
            outfile=(
                f"{path_figures}density_{variable_LE}_{variable_sm}_absSM_summer_HWdays_"
                f"05deg_allregimes_{cond_tag}_{name_land}.png"
            ),
            show_critical_vline=False,
            density_vmax=0.1,
            n_hw_events=n_hw_all,
            spearman_corr=True,
        )

        # Additional: JJA in conditioned years (red) vs rest of years (blue)
        # Same HW-event locations; all JJA days (not HW-only)
        _plot_density_panel(
            all_sm_jja_rest,
            all_LE_jja_rest,
            all_sm_jja_cond,
            ...
            xlabel=r"SMs (regional mean, ±0.5°)",
            title=(
                f"SM vs LE — JJA year types at HW-event locations (absolute SM)\n"
                f"blue: rest-of-years JJA | red: EOF{eof_num}-neg JJA | {cond_tag}"
            ),
            outfile=(
                f"{path_figures}density_{variable_LE}_{variable_sm}_absSM_JJAyears_"
                f"05deg_allregimes_{cond_tag}_{name_land}.png"
            ),
            show_critical_vline=False,
            density_vmax=0.1,
            n_hw_events=n_evt_jja_years,
            blue_cbar_label=f"Rest-of-years JJA density (0–{0.1:.2f})",
            red_cbar_label=f"EOF{eof_num}-neg JJA density",
        )
