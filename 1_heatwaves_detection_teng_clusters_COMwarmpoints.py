...
def match_clusters(clusters_prev, clusters_curr):
    """Greedy one-to-one matching by nearest cluster COM (no overlap required)."""
...
def track_objects_across_days(get_cond, day_indices, lats_arr, lons_arr,
                                ...
                                max_move, day_step=1):
    """
    Track warm clusters day-by-day using nearest-COM assignment.
    Teng condition i: latitude-weighted warm area exceeds min_warm_fraction.
    Teng condition ii: COM of all warm grid cells moves < max_move deg/day.
    Cluster size filter (min_cluster_area_km2) defines event blobs only.
    """
    finished_events = []
    open_tracks = []
    prev_domain_com = None
    ...
    for i, day in enumerate(day_indices):
        ...
        domain_lat, domain_lon = _cluster_com(cond, lats_arr, lons_arr)
        gap = (i == 0) or (day - day_indices[i - 1] != day_step)

        if not gap and open_tracks and prev_domain_com is not None:
            if not _com_motion_ok(
                prev_domain_com[0], prev_domain_com[1], domain_lat, domain_lon, max_move
            ):
                _finalize_tracks(open_tracks, min_duration, finished_events)
                open_tracks = []

        prev_domain_com = (domain_lat, domain_lon)
...
def coord_max_from_event(pos1_i, dur_i, anom, lats_arr, lons_arr):
    """COM of largest warm blob in time-mean positive anomaly (Teng clusters.py)."""
...
def events_from_tracks(finished_events, lats_arr, lons_arr, anom):
    ...
    for track in finished_events:
        ...
        dur_i = len(obs)
        pos1_i = obs[0]['day']
        duration_hw.append(dur_i)
        pos_day1_hw.append(pos1_i)
        lat, lon = _cluster_com(track['onset_mask'], lats_arr, lons_arr)
        coord_hw.append([np.round(lat, 2), np.round(lon, 2)])
        coord_hw_max.append(coord_max_from_event(pos1_i, dur_i, anom, lats_arr, lons_arr))
