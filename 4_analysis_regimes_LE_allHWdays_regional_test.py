...
# Red contours use HW days 0–4 only (onset through day +4), not full duration.
...
# HW sample: event days 0,1,2,3,4 only (capped by duration).
HW_DAY_MAX = 4  # inclusive → days 0..4
lags_days = [5]
...
def _plot_density_panel(
    ...
    if show_hw_contours and has_hw and sm_hw_valid.size > 5:
        ...
        cbar_hw.set_label('HW days 0–4 density', fontsize=10)
...
for region_tag, lon_lo, lon_hi in US_LON_REGIONS:
    ...
    density_cache_path = (
        ...
        f'aroundOnsetloc_summer_HWdays0to4_05deg_{name_land}_{region_tag}.npz'
    )
    print(
    ...
    for lag_days in lags_days:
        ...
        if load_cache:
            ...
            for lat_center_hw, lon_center_hw, onset_day, duration_event in zip(
                ...
                # Full-duration days excluded from summer clim; red sample = days 0–4 only
                n_days_full = max(int(duration_event), 0)
                n_days_sample = min(HW_DAY_MAX + 1, n_days_full)  # 0..4 inclusive
                heatwave_days_full = [
                    onset_day + dt.timedelta(days=i) for i in range(n_days_full)
                ]
                heatwave_days_sample = [
                    onset_day + dt.timedelta(days=i) for i in range(n_days_sample)
                ]
                heatwave_idx = np.where(np.isin(common_dates, heatwave_days_sample))[0]
                hw_mask = np.zeros(len(common_dates), dtype=bool)
                hw_full_idx = np.where(np.isin(common_dates, heatwave_days_full))[0]
                hw_mask[hw_full_idx] = True
        ...
        _plot_density_panel(
            ...
            title=(
                ...
                f'{lon_desc} | Max-anomaly location | HW days 0–4'
            ),
            outfile=(
                f'{path_figures}density_{variable_LE}_{variable_sm}_aroundOnsetloc_absSM_summer_HWdays0to4_'
        ...
        _plot_density_panel(
            ...
            outfile=(
                f'{path_figures}density_{variable_LE}_{variable_sm}_aroundOnsetloc_relBP_summer_HWdays0to4_'
